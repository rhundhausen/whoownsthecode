...
- `content/developments/` is the **Developments** section, a running blotter of court rulings, agency guidance, and industry events (`/developments/`). `layouts/developments/list.html` renders the list newest first inside the editor-panel shell; each entry shows its date, title, and the front-matter `summary` blurb. Post front matter: `date` is the **event** date (used for ordering), `lastmod` is when the entry was written or last edited, `displaydate` (optional, e.g. `"June 2026"`) overrides the formatted date on the list and post when only a month is known, and `summary` is the one- or two-sentence blurb. Posts render through the default single layout, which adds the date line and an "All developments" link for this section only. Every post ends with the standard disclaimer line (copy it from the bottom of `content/faq.md`).
- `content/transcripts/` holds cleaned presentation transcripts. Each file sets `url: /presentations/transcripts/<slug>/` so the pages live under Presentations (and keep the Presentations nav link highlighted); the section's own `_index.md` is `build: render: never` so no bare `/transcripts/` page is emitted. Link each transcript from `content/presentations.md` in the existing `Month | [Transcript](...)` style.
...
- **"Updated" dates.** The footer of every page prints `// Updated: <2 January 2006>` from the page's `.Lastmod` (front matter `lastmod`, falling back to `date`; the homepage takes the newest date across the site). Whenever you edit a content page, set its front-matter `date` (or `lastmod` where `date` means something else, as on Developments posts) to today's date so the footer stays truthful. Do this every time, not only when asked.
